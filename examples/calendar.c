                                     ...
                                     gint          buff_len )
{
  ...
  dtk_calendar_get_date (GTK_CALENDAR (data->window),
			 &year, &month, &day);
                                                 ...
                                                 CalendarData *data )
{
  ...
  if (GTK_CALENDAR (data->window)->marked_date[day-1] == 0) {
    dtk_calendar_mark_day (GTK_CALENDAR (data->window), day);
  } else {
    dtk_calendar_unmark_day (GTK_CALENDAR (data->window), day);
  }
}
...
static void calendar_set_flags( CalendarData *calendar )
{
  ...
  if (calendar->window)
    dtk_calendar_display_options (GTK_CALENDAR (calendar->window), options);
}
...
static void create_calendar( void )
{
  ...
  dtk_calendar_mark_day (GTK_CALENDAR (calendar), 19);
